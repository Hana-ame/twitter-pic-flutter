...
//
// **现状（decoder fallback fork 之后）**：media3 会把"硬解实例占满/规格超出"在
// 播放器内部顺延到软解，报不上来 —— 所以 [onCodecFailure] 目前**没有任何调用方**
// （见 video/decoder_policy.dart）。保留这个原语：它仍是"如果哪天关掉 fallback"
// 的正确策略，且被单测覆盖着。上限收紧到 3 的决策也在 policy 里，不在这里。
//
