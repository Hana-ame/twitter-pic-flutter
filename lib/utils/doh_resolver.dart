...
Future<String> resolveDomainRobustly(String domain) async {
  ...
  final entries = [
    ...
    // 第三条走域名形式 + 严格校验：对冲 _certLooksLike 的正则匹配失败
    // （subject 格式在不同系统上有差异），也让"系统 DNS 可用但 moonchan.xyz
...
/// 是 IP，和证书的 CN 对不上，默认校验必然失败。但"必然失败"不等于"接受一切"
/// —— 这里换成"证书名字必须匹配预期服务域名"，保住名字校验这一层。
///
/// 注意 [X509Certificate] 的 API 只有 `subject` / `issuer` 两个 String
/// （`commonName`、`fingerprint` 这些都不存在，别猜）。subject 是
/// DistinguishedName 串，格式在不同系统上有差异：可能是 RFC 2253 的
/// `CN=doh.pub,O=...`，也可能是 `O=..., CN = doh.pub`（属性顺序反转、等号带空格）。
/// 所以下面按"任意位置出现 CN=值"来取，值取到逗号或换行前为止。
