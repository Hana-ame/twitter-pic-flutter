...
**槽位到底被占多久：抓帧不占，init 占**。`RepaintBoundary.toImage()` 是纯渲染操作
（几十毫秒、不需要解码器），但"抓"必须先有"帧"，而帧是 `initialize()` 解出来的 ——
而 video_player **没有 API** 能告诉我们解码器是哪一刻被真正申请的（ExoPlayer 在
prepare 快结束时才配 MediaCodec）。所以槽位只能从 init 开始按最坏情况占着。
日志里会分别量出两段：
...
- 量出"槽位到底被占多久"：日志新增 `decode: 槽位占用 1.8s（init 1.8s + 抓帧 43ms）`
  —— 直接证明占解码器的是 `initialize()`（网络取 moov + 首帧），抓帧只占几十毫秒
- 槽位在**抓到帧的那一刻就还掉**（不再等写盘完成）
- 并发上限区间 1~4 → **1~6**：撞上限的失败可自愈，放宽的代价小、有能力的设备能更快铺满
