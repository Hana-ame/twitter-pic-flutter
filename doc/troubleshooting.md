...
**槽位被占多久**：日志里每次抓帧都会记一条 `decode: 槽位占用 …（init … + 抓帧 …）`。
`init` 是网络耗时（取 moov + 首帧），抓帧是渲染操作 —— 两者通常差 1~2 个量级。
**抓封面本身不占解码器**，占着槽位的是"必须先有一个解码出来的帧"这件事；
而 video_player 没有 API 能报告解码器实际被申请的时机，所以只能按最坏情况占着。

**并发上限不是写死的 2**：`utils/decode_budget.dart` 从 2 起步，连续抓帧成功 3 次
上调 1、撞 MediaCodec 失败立即下调 1，范围 1~6，学到的值存档。日志里能看到本机实测：
